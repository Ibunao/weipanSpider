...
class WeipandirSpider(scrapy.Spider):
    ...
    def my_process_next(self, response):
        ...
        return Request(target_url+str(self.page[response.meta['filepath']]), meta={'cookiejar': response.meta['cookiejar'], 'filepath': response.meta['filepath']}, callback=self.parse)
    ...
    def process_dir(self, info, response):
        ...
        # 保存不同目录的链接
        self.dirtarget_url[filepath] = info['link']+'&pn='
